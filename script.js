let demons = JSON.parse(localStorage.getItem('demons_simple')||'null') || [
  {
    nom:'Zirossi',
    pouvoir:'Résonance',
    niveau:'Puissant',
    ...
    age:'Non renseigné',
    recherches:'Aucun',
    observations:'Démon lié aux vibrations sonores.',
    image:''
  }
];

let logs = JSON.parse(localStorage.getItem('logs_simple')||'null') || [];
let imageData = '';

function save(){
  localStorage.setItem('demons_simple',JSON.stringify(demons));
  localStorage.setItem('logs_simple',JSON.stringify(logs));
}
...
resetAll.onclick = () => {
  if(confirm('Réinitialiser toutes les fiches ?')){
    localStorage.removeItem('demons_simple');
    localStorage.removeItem('logs_simple');
    location.reload();
  }
};
